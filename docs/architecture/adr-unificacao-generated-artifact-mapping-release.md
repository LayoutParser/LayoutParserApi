...
- **Status:** Aceito — backend implementado (ver §7)
- **Data:** 2026-09-18
...
## 7. Implementação (backend) — desvios e decisões provisórias

Implementado em `MappingGovernanceController.List` + `GeneratedMapperListService`
(`Services/Transformation/Ai`) + `IGeneratedMapperArtifactStore.ListAsync`. Nada migra de tabela.

- **Vínculo mapper→workspace (#417) — PROVISÓRIO, opção (a), sem isolamento:** todo artefato
  gerado aparece para todo workspace, porque o catálogo Sysmiddle já é org-wide
  (`AllowedPackageGuids`). Não fecha o caminho para isolar depois: o ponto único de filtragem é
  `IGeneratedMapperListService` (passaria a receber `workspaceId`). Reavaliar quando #417 fechar.
- **Paginação (§3.2):** em vez de "bloco fixo na 1ª página" (que estouraria `pageSize`), a lista é uma
  sequência única "releases reais (recentes primeiro) e depois auto-gerados (recentes primeiro)";
  `offset` é sempre múltiplo de `pageSize`, então a página de releases do store segue alinhada.
  `totalCount` soma as duas fontes. O merge ordenado por timestamp continua adiado.
- **Filtros:** `origin` novo. `draftId`/`environment` excluem os auto-gerados. `status` aceita o ciclo
  de vida da release (exclui auto-gerados) ou `ready`/`generating`/`stale` (exclui releases).
  `stale` não é persistido nem calculado na lista (exigiria decifrar/parsear todo `MapperVo`) — só o
  detalhe o calcula; a lista mostra o status persistido.
- **Item auto-gerado:** sem `engine` (o candidato é único/combinado, ver §3.1) e sem `content` (padrão
  da lista de releases); traz `detailUrl` para `GET .../mappings/{mapperGuid}/generated-transformation`.
  Nota: esse detalhe dispara regeração lazy se o mapper mudou (`stale`) — comportamento herdado.
- **Nome (`mapperName`):** via `ICachedMapperService.GetAllMappersAsync` (cache Redis; cai para
  `tbMapper`, SOMENTE LEITURA). Falha → item sem `mapperName`, segue com `mapperGuid`.
- **Resiliência:** store de auto-gerados falhando não derruba a lista; devolve só as reais e
